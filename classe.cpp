...
private:
    T nodi[100];  
    bool presente[100];  //array per tenere traccia dei nodi inseriti

public:
    AlberoBinario() {
        // Inizializza tutto l'albero come vuoto
        for (int i = 0; i < 100; ++i) {
            presente[i] = false;
        }
    }
    ...
    bool alberoVuoto() const {
        return !presente[0];  // la radice non è presente
    }
    ...
    bool isFoglia(int indice) const {
        return (2 * indice + 1 >= 100 || !presente[2 * indice + 1]) &&  // figlio sinistro assente
               (2 * indice + 2 >= 100 || !presente[2 * indice + 2]);    // figlio destro assente
    }
